...
@pytest.mark.ui
def test_javascript_scroll_to_footer(browser, base_url):
    page = HomePage(browser, base_url).open()
    page.scroll_to(HomePage.FOOTER)
    scroll_y = browser.execute_script("return window.pageYOffset;")
    assert scroll_y > 0


@pytest.mark.ui
def test_browser_cookie_can_be_added_and_read(browser, base_url):
    HomePage(browser, base_url).open()
    browser.add_cookie({"name": "qa_cookie", "value": "selenium_test"})
    cookie = browser.get_cookie("qa_cookie")
    assert cookie["value"] == "selenium_test"
